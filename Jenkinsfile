...
def kind_k8s_map = [
    '0.31.0': [
        ...
        '1.32.11': 'kindest/node:v1.32.11@sha256:5fc52d52a7b9574015299724bd68f183702956aa4a2116ae75a63cb574b35af8',
        '1.32':    'kindest/node:v1.32.11@sha256:5fc52d52a7b9574015299724bd68f183702956aa4a2116ae75a63cb574b35af8',
        '1.31.14': 'kindest/node:v1.31.14@sha256:6f86cf509dbb42767b6e79debc3f2c32e4ee01386f0489b3b2be24b0a55aac2b',
...
EOF
                            ...
                            # Configure local registry access on each node using hosts.toml
                            for node in $(kind get nodes --name "${kind_name}"); do
                                podman exec ${node} mkdir -p /etc/containerd/certs.d/localhost:${registry_port}
                                podman exec ${node} bash -c "cat > /etc/containerd/certs.d/localhost:${registry_port}/hosts.toml <<TOML
