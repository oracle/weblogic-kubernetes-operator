 ...
 *  `involvedObject`:  `V1ObjectReference` object that describes the Kubernetes resources with which this event is associated.
    *  `name`:  String that describes the name of the resource that the event is associated with. It may be the `domainUID`, the name of the namespace that the operator watches, or the operator pod name.
    ...
    *  `kind`:  String that represents the REST resource this object represents. The value is `Domain` for a domain event, `Namespace` for a namespace event in the domain namespace, or the `Pod` for an operator pod name.
    *  `apiVersion`:  String that describes the `apiVersion` of the involved object, which is the `apiVersion` of the domain resource, for example, `weblogic.oracle/v8`, for a domain event, or, apiVersion of V1Namespace, for a namespace event. When the involvedObject is the operator pod, the value is the Kubernetes V1Pod apiVersion.
