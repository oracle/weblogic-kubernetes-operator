...
To obtain the image, login to the Oracle Container Registry:
...
database in Kubernetes. For more details about database set up and configuration, refer to this [page]({{< relref "/userguide/managing-fmw-domains/fmw-infra/_index.md#running-the-database-inside-kubernetes" >}}).

When running the Oracle database in Kubernetes, you have to option
...
These instructions will set up the database in a container with persistent volume (PV) attached. 
If you chose not to use persistent storage, please go to [RCU creation step](#running-the-repository-creation-utility-to-set-up-your-database-schema).

* Create the persistent volume and persitent volume claim for the database
using the [create-pv-pvc.sh]({{< relref "/samples/simple/storage/_index.md" >}}) sample.
Please refer to the instructions provided in that sample.

{{% notice note %}}
When creating the PV and PVC for the database, make sure that you use a different name
and storage class to the PV and PVC for the domain.
...
>NOTE: Make sure you update `kubernetes/samples/scripts/create-soa-domain/domain-home-on-pv/create-database/db-with-pv.yaml` 
>file with the name of the PVC created in the previous step. Also update the value for all the occurrences of namespace field 
...
You will see a message in the log telling you when the database is ready.  You can also 
verify that the database service status is ready using this command:
...
Below is an example of the commands you might use to execute `create-rcu-schema.sh`:
...
Below is an example if the commands you might use to execute `drop-rcu-schema.sh`:
...
For SOA domains `drop-rcu-schema.sh` script supports the domain types `soa,osb,soaosb,soaess,soaessosb`,
you must specify one of these using the `-t` flag.
...
to create your domain.  Please continue to follow the instructions in the [SOA Domain sample]({{< relref "/samples/simple/domains/soa-domain/_index.md" >}}).

#### Monitoring a SOA domain

After the SOA domain is set up, you can also:
