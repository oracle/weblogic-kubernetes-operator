...
| `httpAccessLogInLogHome` | Boolean | If true (the default), the server HTTP access log file will be written to the same directory specified in logHome. Otherwise, server HTTP access log file will be written to the directory as configured in the WebLogic configuration for each server. |
...
| `logHome` | string | The in-pod name of the directory in which to store the domain, node manager, server logs, server  *.out, and optionally HTTP access log files |
