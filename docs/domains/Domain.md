...
| `maxConcurrentShutdown` | number | The maximum number of Managed Servers instances that the operator will shutdown in parallel for this cluster in response to a change in the `replicas` count. If more Managed Server instances must be shutdown, the operator will wait until a Managed Server Pod is terminated before shutting down the next Managed Server instance. A value of 0 means all Managed Server instances will shutdown in parallel. Defaults to 1. |
