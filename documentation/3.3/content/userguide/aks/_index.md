...
| Is the specified SSO account associated with an active Oracle support contract? | If set to **Yes**, you must accept the license agreement in `middleware/weblogic_cpu` repository. If set to **No**, you must accept the license agreement in `middleware/weblogic`. Steps to accept the license agreement: log on to [Oracle Container Registry](https://container-registry.oracle.com/); navigate to repository `middleware/weblogic_cpu` and `middleware/weblogic`; accept license agreement. See this [document](https://aka.ms/wls-aks-ocr-doc) for more information. |
