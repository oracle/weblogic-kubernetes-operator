...
- *Can you make the desired change with a configuration override or Model in Image ConfigMap?*  
  ...
  for Domain on PV and Domain in Image, and with model updates for Model in Image.
  ...
  for Domain on PV and Domain in Image, and use model updates for Model in Image.
...
- *Do you need to be able to do a rolling restart?*
  ...
  If you need to do a rolling restart with Model in Image or Domain on PV, the domain
  encryption keys are not a concern. In Model in Image, the keys are generated by
  the operator at runtime before the first WebLogic Server pod is started. In Domain on PV,
