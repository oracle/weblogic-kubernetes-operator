...
| `httpRequestFailureCount` | integer | Non-zero if the HTTP request to get the server's health fails for any reason. The HTTP request might fail when the server is in the process of shutting down. |
