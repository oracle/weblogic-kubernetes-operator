...
| `cluster-resource-references` | Array of string | List of names of Cluster Resources the Operator expects to be deployed in the same namespace that the domain is deployed.  A reference to a cluster resource  in order for the cluster elements to be applied. |
| `clusters` | Array of [Cluster Spec](#cluster-spec) | Lifecycle options for all of the Managed Server members of a WebLogic cluster, including Java options, environment variables, additional Pod content, and the ability to explicitly start, stop, or restart cluster members. The `clusterName` field of each entry must match a cluster that already exists in the WebLogic domain configuration. |
...
| `failureRetryIntervalSeconds` | number | The wait time in seconds before the start of the next retry after a Severe failure. Defaults to 120. |
| `failureRetryLimitMinutes` | number | The time in minutes before the operator will stop retrying Severe failures. Defaults to 1440. |
...
| `logHomeLayout` | string | Control how log files under `logHome` are organized when logHome is set and `logHomeEnabled` is true. `FLAT` - all files directly in the `logHome` root directory. `BY_SERVERS` (default) - domain log files and `introspector.out` are at the `logHome` root level, all other files are organized under the respective server name logs directory  `logHome/servers/<server name>/logs`. |
