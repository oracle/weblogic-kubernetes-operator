...
| `waitForAllSessions` | Boolean | For graceful shutdown only, set to true to wait for all HTTP sessions during inflight work handling; false to wait for non-persisted HTTP sessions only. Defaults to false. |
