...
| `localhostBindingsEnabled` | Boolean | When `true` the operator creates a WebLogic network access point with a `localhost` binding for each existing channel and protocol. This must be set to true when using Istio versions prior to 1.10 and must be set to false for version 1.10 and later. Defaults to true. |
