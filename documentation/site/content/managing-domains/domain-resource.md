...
* The following behavior occurs depending on whether or not `NODEMGR_JAVA_OPTIONS` and `NODEMGR_MEM_ARGS` are defined:
  ...
  * If `NODEMGR_MEM_ARGS` is not defined, then it will use the memory settings in `USER_MEM_ARGS` if it is also defined, otherwise the Java security property values (`-Xms64m -Xmx100m -Djava.security.egd=file:/dev/./urandom`) will be applied to the Node Manager instance. It can be explicitly set to another value in your Domain or Cluster YAML file using the `env` attribute under the `serverPod` configuration.
