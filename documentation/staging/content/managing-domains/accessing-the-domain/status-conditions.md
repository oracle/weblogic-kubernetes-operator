...
The WebLogic Kubernetes Operator populates status conditions on Domain and Cluster resource to provide
...
- `Failed`
    * The desired state of the domain resource cannot be achieved due to failures encountered in
      ...
      See [debugging]({{< relref "managing-domains/debugging.md" >}}) for information about how to diagnose failures.
...
- `ConfigChangesPendingRestart`
  * This condition tracks the progress of [runtime updates]({{< relref "managing-domains/model-in-image/runtime-updates">}})
    to the WebLogic Deploy Tool model of a Model in Image domain home source type domain.
...
Once the operator detects a cluster or domain resource, it will ensure that each resource always has
...
as appropriate. You can use this fact as a way to detect when an operator has detected a
...
condition has changed. See [operator-generated event types]({{< relref "managing-domains/accessing-the-domain/domain-events#operator-generated-event-types">}}) for details.
