...
3.  Use `helm` to install and start the operator from the directory you just cloned:	 
    ...
    **Note:** Prior to version 3.1.0, the operator's Helm chart supported configuring only the namespaces that the operator would manage using a list of namespaces. The chart now supports specifying namespaces using a label selector, regular expression, or list. Review the choices for selecting a domain namespace strategy in the [Namespace management]({{< relref "/userguide/managing-operators/namespace-management.md" >}}) document.
