...
weight: 6
description: "Obtain and inspect images for WebLogic Server or Fusion Middleware Infrastructure deployments."
---
...
Note that all of the OCR images that are described in this document are built using the WebLogic Image Tool (WIT). You can also use WIT to build your own WebLogic Server or Fusion Middleware Infrastructure images (with the latest Oracle Linux images, Java updates, and WebLogic Server patches), apply one-off patches to existing OCR images, or overlay your own files and applications on top of an OCR image. For information about using WIT to create custom WebLogic Server or Fusion Middleware Infrastructure images for the WebLogic Kubernetes Operator, see [Create custom images]({{< relref "/userguide/base-images/custom-images.md" >}}).
