...
### Automated upgrade of WKO 3.x auxiliary images configuration
...
The 4.0 Operator provides a seamless upgrade of the Domains with WKO 3.x (V8 schema) auxiliary images configuration. When you create a Domain with WKO 3.x (V8 schema) style auxiliary images configuration in a namespace managed by the WKO 4.0, the [WebLogic Domain resource conversion webhook]({{< relref "/userguide/managing-operators/conversion-webhook.md" >}}) performs an automated upgrade of the domain resource to 4.0 schema. The conversion webhook runtime converts the WKO 3.x auxiliary image configuration to the equivalent configuration using init-containers, volume and volume mounts under the `serverPod` section in WKO 4.0. Similarly, when [upgrading the Operator version from 3.x to 4.0]({{< relref "/userguide/managing-operators/installation#upgrade-the-operator" >}}), Domains with auxiliary images are seamlessly upgraded.

Below is a sample domain resource stanza for auxiliary image configuration in WKO 3.x (V8 schema) and 
the equivalent configuration generated by the conversion webhook in WKO 4.0.

##### Sample WKO 3.x (V8 schema) auxiliary image configuration:
...
##### Compatibility auxiliary image configuration generated by conversion webhook in WKO 4.0:
