...
### Contents
 ...
 - [Configuration](#configuration)
   ...
   - [Backward compatibility and upgrade](#backward-compatibility-and-upgrade)
     - [Sample WKO 3.x auxiliary image configuration and converted WKO 4.0 configuration](#sample-wko-3x-auxiliary-image-configuration-and-converted-wko-40-configuration)
   - [Upgrading the domain resource manually using domain upgrade tool](#upgrading-the-domain-resource-manually-using-domain-upgrade-tool)
 - [Sample](#sample)
...
The auxiliary image feature has been enhanced in Operator version 4.0 to improve the ease of use significantly, and its configuration has changed. The auxiliary images are now exclusively used for the Model-in-Image Domain use cases. It supports the directories and file types necessary for the Model-in-Image use case (e.g., the `WDT install home` and `WDT model home` directories and the file types for the WDT installation and model files).

When you create a Domain using WKO 3.x/V8 schema Domain resource with the auxiliary image configuration in a namespace that is managed by the WKO 4.0, the [WebLogic Domain custom resource coversion webhook]({{< relref "/userguide/managing-operators/conversion-webhook.md" >}}) automatically upgrades the WKO 3.x auxiliary images. The upgrade is entirely automatic and transparent, and you don't need to take any extra steps. The conversion webhook runtime converts the WKO 3.x/v8-schema auxiliary image configuration to the init-containers, volume and volume mounts configuration under the `serverPod` section of the spec in WKO 4.0/V9 schema. 

#### Sample WKO 3.x auxiliary image configuration and converted WKO 4.0 configuration
Below is a sample domain resource stanza for auxiliary image configuration in WKO 3.x/V8 schema and 
the equivalent configuration generated by the conversion webhook in WKO 4.0/V9 schema domain resource.

##### Sample WKO 3.x/V8 schema auxiliary image configuration to be converted to WKO 4.0/V9 schema:
...
##### Equivalent compatibility auxiliary image configuration generated by the conversion webhook in the WKO 4.0/V9 schema domain resource:
...
### Upgrading the domain resource manually using domain upgrade tool
If you want to manually upgrade the domain resource from the 3.x schema to the 4.0 schema, refer to [upgrade the 3.x domain custom resource manually]({{< relref "/userguide/managing-operators/installation#upgrade-the-3x-domain-custom-resource-manually" >}}) section in the [installation and upgrade]({{< relref "/userguide/managing-operators/installation.md" >}}).
