    ...
    public class ITOperatorHelmChart extends BaseTest {
        ...
        public void testOperatorCreateDeleteCreate() throws Exception {
            ...
            try{
                ...
                if (firstoperator == null) {
                    firstoperator =
                            TestUtils.createOperator(operator2File);
                }
                secondoperator = new Operator((TestUtils.createOperatorMap(number, true)), RESTCertType.SELF_SIGNED);
                ...
                logger.info("Create again second operator pod and verify it is started again after create - delete -create steps");
                secondoperator = new Operator((TestUtils.createOperatorMap(number, true)),false,false,false, RESTCertType.SELF_SIGNED);
                secondoperator.callHelmInstall();
        ...
        public void testCreateSecondOperatorUsingSameOperatorNS() throws Exception {
            ...
            try{
                ...
                if(!ex.getMessage().contains("Error: release " + oprelease + " failed: secrets \"weblogic-operator-secrets\" already exists")){
                    throw new RuntimeException(
                            "FAILURE: Helm install operator with dublicated target domains namespaces does not report expected message " + ex.getMessage());
                };
        ...
        public void testNotExistedOpNSCreateOperatorNegativeInstall() throws Exception {
            ...
            try {
                operator = new Operator((TestUtils.createOperatorMap(number, false)),false,false,true, RESTCertType.NONE);
                operator.callHelmInstall();
                throw new RuntimeException(
                        "FAILURE: Helm install operator with with not existed namespace ");
                ...
                if (result.exitValue() != 0) {
                    throw new RuntimeException(
                            "FAILURE: Helm install operator with not existed namespace ");
                }
            }
        ...
        public void testNotExistedOpServiceAccountCreateOperatorNegativeInstall() throws Exception {
            ...
            Operator operator = null;
            try {
                operator = new Operator((TestUtils.createOperatorMap(number, false)),true,false,true, RESTCertType.NONE);
                operator.callHelmInstall();
        ...
        /**
         * Negative test : Operator create second operator with same target domain namespace as the first operator
         * @throws Exception
         */
        @Test
        public void testSecondOpSharingSameTargetDomainsNSNegativeInstall() throws Exception {
            ...
            try {
                ...
                ArrayList<String> targetDomainsNS = (ArrayList<String>)firstoperator.getOperatorMap().get("domainNamespaces");
                ...
                if(!ex.getMessage().contains("Error: release " + oprelease + " failed: rolebindings.rbac.authorization.k8s.io \"weblogic-operator-rolebinding-namespace\" already exists")){
                    throw new RuntimeException(
                            "FAILURE: Helm install operator with dublicated target domains namespaces does not report expected message " + ex.getMessage());
                };
                String cmdLb = "helm list --failed " + "  | grep " + oprelease;
        ...
        public void testTargetNSIsNotCreatedNegativeInstall() throws Exception {
            ...
            Operator operator= null;
            try {
                operator = new Operator(TestUtils.createOperatorMap(number,false),true,true,false, RESTCertType.NONE);
                operator.callHelmInstall();
                throw new RuntimeException(
                        "FAILURE: Helm install operator with not precreated target domains namespaces ");

            }catch (Exception ex) {
                if(!ex.getMessage().contains("Error: release " + oprelease + " failed: namespaces \"test" + number + "\" not found")){
                    throw new RuntimeException(
                            "FAILURE: Helm install operator with not precreated target domains namespaces does not report expected message " + ex.getMessage());
                };
                String cmdLb = "helm list --failed " + "  | grep " + oprelease;
                logger.info("Executing cmd " + cmdLb);
                ExecResult result = ExecCommand.exec(cmdLb);
        ...
        /**
         * Negative test : Operator create operator with dublicated ExternalRestPort, use upgrade to update to correct value
         * @throws Exception
         */
        @Test
        public void testSecondOpSharingSameExternalRestPortNegativeInstall() throws Exception {
            ...
            try {
                ...
                operator2 =
                        new Operator(operatorMap, RESTCertType.SELF_SIGNED);
                ...
                if(!ex.getMessage().contains("Service \"external-weblogic-operator-svc\" is invalid: spec.ports[0].nodePort: Invalid value:")){
                    throw new RuntimeException(
                            "FAILURE: Helm install operator with dublicated rest port number does not report expected message " + ex.getMessage());
                };
                String cmdLb = "helm list --failed " + "  | grep " + oprelease;
                logger.info("Executing cmd " + cmdLb);
                ExecResult result = ExecCommand.exec(cmdLb);
        ...
        /**
         * Negative test : Operator create operator with invalid target domains namespaces existed operator namespace
         * @throws Exception
         */
        @Test
        public void testCreateWithUpperCaseTargetDomainNegativeInstall() throws Exception {
            ...
            try {
                ...
                if(!ex.getMessage().contains("Error: release " + oprelease + " failed: namespaces \"Test9\" not found")){
                    throw new RuntimeException(
                            "FAILURE: Helm install operator with UpperCase for target domains namespace does not report expected message " + ex.getMessage());
                };
                String cmdLb = "helm list --failed " + "  | grep " + oprelease;
                logger.info("Executing cmd " + cmdLb);
                ExecResult result = ExecCommand.exec(cmdLb);
        ...
        public void testCreateChartWithInvalidAttributesNegativeInstall() throws Exception {
            ...
            try {
                ...
                if (!ex.getMessage().contains("java.lang.String cannot be cast to java.lang.Boolean")) {
                    throw new RuntimeException(
                            "FAILURE: Helm install operator with invalid value for attribute externalRestEnabled does not report expected message " + ex.getMessage());
                }
                ;
            ...
            try {
                ...
                if (!ex.getMessage().contains("javaLoggingLevel must be one of the following values [SEVERE WARNING INFO CONFIG FINE FINER FINEST] : INVALIDOPTION")) {
                    throw new RuntimeException(
                            "FAILURE: Helm install operator with invalid value for attribute externalRestEnabled does not report expected message " + ex.getMessage());
                }
            }
