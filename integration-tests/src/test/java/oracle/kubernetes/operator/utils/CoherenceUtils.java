...
public class CoherenceUtils {
  ...
  private void setCohEnv() {

    try {
      URL url = getClass().getResource("/coherence/proxy-client-cache-config.xml");
      clientCacheConfigPath = Paths.get(url.toURI());
    ...
    System.setProperty("tangosol.coherence.management.refresh.policy", "refresh-behind");
  ...
  private abstract static class CacheThread extends Thread {
    ...
    public void run() {
      ...
      for (int i = 0; i < VALUE_SIZE; i++) valBuf.append('a');
