...
sh /var/scripts/scalingAction.sh --action=scaleDown --domain_uid=test1 --cluster_name=cluster-1 --kubernetes_master=$MASTER --wls_domain_namespace=test5 --operator_namespace=weblogic-operator1  2>&1 | tee -a scaleDnAction.log
