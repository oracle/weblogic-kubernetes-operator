...
function validateClusterName {
  clusterNameLC=$(toLowerDNS1123Legal $clusterName)
}
...
function validateAdminServerName {
  adminServerNameLC=$(toLowerDNS1123Legal $adminServerName)
}
...
function validateManagedServerNameBase {
  managedServerNameBaseLC=$(toLowerDNS1123Legal $managedServerNameBase)
}
...
function createYamlFiles {
  ...
  sed -i -e "s:%ADMIN_SERVER_NAME_LC%:${adminServerNameLC}:g" ${createJobOutput}
  ...
  sed -i -e "s:%MANAGED_SERVER_NAME_BASE_LC%:${managedServerNameBaseLC}:g" ${createJobOutput}
