...
```  
  ...
  -p password for schema owner (regular user), must be provided through -p argument or user will be prompted to enter a value.
  -a username for SYSDBA user, must be specified.
  -q password for SYSDBA user, must be provided through -q argument or user will be prompted to enter a value.
