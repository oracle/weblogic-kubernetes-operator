...
function usage {
  echo usage: ${script} -i file -o dir [-e] [-d] [-h]
...
function createYamlFiles {
  ...
  sed -i "/image:/c\image: ${weblogicDockerImage}" ${domain1Output}
  sed -i "/#imagePullSecretName:/c\imagePullSecretName: ${imagePullSecretName}" ${domain1Output}
  sed -i "/imagePullSecretName:/c\imagePullSecretName: ${imagePullSecretName}" ${domain1Output}
  sed -i "/exposeAdminNodePort:/c\exposeAdminNodePort: true" ${domain1Output}
  sed -i "/persistentVolumeClaimName:/c\persistentVolumeClaimName: ${persistentVolumeClaimName}" ${domain1Output}
...
function waitForJobComplete {
   attempts=0
   svcState="running"
   while [ ! "$svcState" == "completed" ] && [ ! $attempts -eq 20 ]; do
    svcState="completed"
    attempts=$((attempts + 1))
...
if [ "${executeIt}" = true ]; then
  ...
  export azureResourceGroupName="${azureResourceNamePrefix}resourcegroup${azureResourceUID}"
  export aksClusterName="${azureResourceNamePrefix}akscluster${azureResourceUID}"
  export storageAccountName="${azureResourceNamePrefix}storage${azureResourceUID}"
