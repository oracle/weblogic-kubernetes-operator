...
#
#Adopted from https://github.com/oracle/docker-images/blob/main/OracleWebLogic/samples/12213-domain-home-in-image/container-scripts/startAdminServer.sh
#
...
SEC_PROPERTIES_FILE=${PROPERTIES_FILE_DIR}/domain.properties
...
# Get Username
USER=`awk '{print $1}' ${SEC_PROPERTIES_FILE} | grep ADMIN_USER_NAME | cut -d "=" -f2`
if [ -z "${USER}" ]; then
   echo "The domain username is blank.  The Admin username must be set in the properties file."
   exit
fi
# Get Password
PASS=`awk '{print $1}' ${SEC_PROPERTIES_FILE} | grep ADMIN_USER_PASS | cut -d "=" -f2`
if [ -z "${PASS}" ]; then
