...
The `introspectDomain.sh` script can be used to explicitly initiate the introspection of a WebLogic domain managed by the operator.  This script updates the value of `spec.introspectVersion` attribute of the domain resource to explicitly initiate the introspection. You can provide the new value for `spec.introspectVersion` as a parameter to the script or the script will automatically generate a new value to trigger the introspection. See the script `usage` information by using the `-h` option.
