...
public class MySQLDBUtils {
  ...
  private void startMySQLDB(String name, String namespace) {
  ...
  private void createSecret(String namespace) throws ApiException {
    HashMap<String, byte[]> secrets = new HashMap<>();
    secrets.put("root-user", Base64.getEncoder().encode("root".getBytes()));
    secrets.put("root-password", Base64.getEncoder().encode("root123".getBytes()));

    boolean secret = TestActions.createSecret(
        new V1Secret()
            .metadata(new V1ObjectMeta()
                .name("mysql-secret")
  ...
  private void createService(String namespace, int port) throws ApiException {
    boolean service = TestActions.createService(new V1Service()
        .metadata(new V1ObjectMeta()
            .name("mysql")
            .namespace(namespace))
