...
public class Main {
  ...
  private static final ServerKubernetesObjectsFactory skoFactory =
      ServerKubernetesObjectsFactory.getInstance();
  private static final DomainPresenceInfoFactory domains = DomainPresenceInfoFactory.getInstance();
  ...
  static Map<String, DomainPresenceInfo> getDomainPresenceInfos() {
    return domains.getDomainPresenceInfos();
  }
    ...
    try {
      ...
      engine
          ...
          .scheduleWithFixedDelay(
              updateDomainPresenceInfos(domains.getDomainPresenceInfos().values()),
              recheckInterval,
              recheckInterval,
              TimeUnit.SECONDS);
    } catch (Throwable e) {
  ...
  private static void deleteStrandedResources() {
    for (Map.Entry<String, DomainPresenceInfo> entry :
        domains.getDomainPresenceInfos().entrySet()) {
  ...
  public static void doRestartAdmin(String principal, String domainUID) {
    DomainPresenceInfo info = domains.lookup(domainUID);
    if (info != null) {
  ...
  private static void doCheckAndCreateDomainPresence(
    ...
    DomainPresenceInfo info = domains.getOrCreate(dom);
    ...
    if (ref != null) {
      ...
      if (message != null) {
        if (message.contains(WebLogicConstants.READINESS_PROBE_NOT_READY_STATE)) {
          ServerKubernetesObjects sko = skoFactory.lookup(name);
          if (sko != null) {
      ...
      if (result != null) {
        for (V1beta1Ingress ingress : result.getItems()) {
          ...
          if (domainUID != null && clusterName != null) {
            domains.getOrCreate(ns, domainUID).getIngresses().put(clusterName, ingress);
          }
        }
      }
      ingressWatchers.put(
      ...
      if (result != null) {
        for (V1Service service : result.getItems()) {
          ...
          if (domainUID != null && serverName != null) {
            DomainPresenceInfo info = domains.getOrCreate(ns, domainUID);
            ServerKubernetesObjects sko = skoFactory.getOrCreate(info, domainUID, serverName);
            if (channelName != null) {
