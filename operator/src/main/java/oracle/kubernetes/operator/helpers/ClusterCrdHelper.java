...
public class ClusterCrdHelper {
  ...
  interface CrdComparator {
    boolean isOutdatedCrd(
        SemanticVersion productVersion, V1CustomResourceDefinition actual,
        V1CustomResourceDefinition expected);
  }
      ...
      private boolean existingCrdContainsVersion(V1CustomResourceDefinition existingCrd) {
        ...
        if (versions != null) {
          for (V1CustomResourceDefinitionVersion v : versions) {
            if (KubernetesConstants.CLUSTER_VERSION.equals(v.getName())) {
              found = true;
              break;
            }
          }
  ...
  static class CrdComparatorImpl implements CrdComparator {
    ...
    public boolean isOutdatedCrd(SemanticVersion productVersion,
      ...
      ResourceVersion current = new ResourceVersion(KubernetesConstants.CLUSTER_VERSION);
