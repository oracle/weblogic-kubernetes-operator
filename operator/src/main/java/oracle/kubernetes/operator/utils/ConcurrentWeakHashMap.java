// Copyright 2009, 2018, Oracle Corporation and/or its affiliates.  All rights reserved.
 ...
 * operations.
 *
 * <p>While keys in this table are only held using a weak reference, values are held using a normal
 ...
 *
 * <p>Just like {@link java.util.concurrent.ConcurrentHashMap}, this class obeys the same functional
 * specification as {@link java.util.Hashtable}, and includes versions of methods corresponding to
 * each method of <tt>Hashtable</tt>. However, even though all operations are thread-safe, retrieval
 * operations do <em>not</em> entail locking, and there is <em>not</em> any support for locking the
 * entire table in a way that prevents all access. This class is fully interoperable with
 * <tt>Hashtable</tt> in programs that rely on its thread safety but not on its synchronization
 * details.
 *
  ...
  static final class Segment<K, V> extends ReentrantLock implements Serializable {
    ...
    boolean containsKey(Object key, int hash) {
      if (count != 0) { // read-volatile
        HashEntry<K, V> e = getFirst(hash);
        while (e != null) {
          if (e.hash == hash && key.equals(e.keyRef.get())) return true;
          e = e.next;
        }
      }
      return false;
    ...
    boolean replace(K key, int hash, V oldValue, V newValue) {
      ...
      try {
        removeStale();
        HashEntry<K, V> e = getFirst(hash);
        while (e != null && (e.hash != hash || !key.equals(e.keyRef.get()))) e = e.next;
    ...
    V remove(Object key, int hash, Object value, boolean weakRemove) {
      ...
      try {
        ...
        while (e != null
            && (!weakRemove || key != e.keyRef)
            && (e.hash != hash || !key.equals(e.keyRef.get()))) e = e.next;
  ...
  abstract class HashIterator {
    ...
    public boolean hasNext() {
      while (nextEntry != null) {
        if (nextEntry.keyRef.get() != null) return true;
        advance();
      }
