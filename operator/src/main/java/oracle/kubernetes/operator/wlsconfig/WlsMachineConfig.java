...
public class WlsMachineConfig {
  ...
  public WlsMachineConfig(String name, Integer nodeManagerListenPort, String nodeManagerListenAddress, String nodeManagerType) {
    this.name = name;
  ...
  /**
   * Creates a WlsMachineConfig object using an "machines" item parsed from JSON result from WLS REST call
   *
   * @param machineConfigMap Map containing "machine" item parsed from JSON result from WLS REST call
   *
  ...
  static WlsMachineConfig create(Map<String, Object> machineConfigMap) {
    ...
    return new WlsMachineConfig(machineName, nodeManagerListenPort, nodeManagerListenAddress, nodeManagerType);
  }
  ...
  static String getSearchPayload() {
    return  "   fields: [ " + getSearchFields() + " ], " +
            ...
            "   children: { " +
            "      nodeManager: { " +
            "      fields: [ " + getNodeManagerSearchFields() + " ], " +
            "      links: [] " +
            "        }" +
            "    } ";
  }
   ...
   * request.
   *
   * @return A string containing machine configuration fields that should be retrieved from the WLS REST
   ...
   * request.
   *
   * @return A string containing node manager configuration fields that should be retrieved from the WLS REST
   *         request, in a format that can be used in the REST request payload
   */
