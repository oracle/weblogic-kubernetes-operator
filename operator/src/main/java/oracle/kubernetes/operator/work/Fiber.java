 ...
 * The {@link Engine} is capable of running a large number of flows concurrently
 * by using a relatively small number of threads. This is made possible by
 * utilizing a {@link Fiber} &mdash; a user-level thread that gets created for
 * each processing flow.
 * A fiber remembers where in the pipeline the processing is at and other
 * additional information specific to the execution of a particular flow.
 * <h2>Suspend/Resume</h2>
 * Fiber can be {@link NextAction#suspend(Consumer) suspended} by a {@link Step}. When a
 * fiber is suspended, it will be kept on the side until it is
 * {@link #resume(Packet) resumed}. This allows threads to go execute other
 * runnable fibers, allowing efficient utilization of smaller number of threads.
 * <h2>Context ClassLoader</h2>
 * Just like thread, a fiber has a context class loader (CCL.) A fiber's CCL
 * becomes the thread's CCL when it's executing the fiber. The original CCL of
 * the thread will be restored when the thread leaves the fiber execution.
 * <h2>Debugging Aid</h2>
 * Setting the {@link #LOGGER} for FINE would give you basic
 * start/stop/resume/suspend level logging. Using FINER would cause more
 * detailed logging, which includes what steps are executed in what order and
 * how they behaved.
 */
public final class Fiber implements Runnable, Future<Void>, ComponentRegistry {
  ...
  /**
   * If this field is non-null, the next instruction to execute is to call its
   ...
   * Logically this is just a boolean, but we need to prepare for the case where
   * the thread is {@link #resume(Packet) resumed} before we get to the
   * {@link #suspend()}. This happens when things happen in the following order:
   * <ol>
   * <li>Step decides that the fiber needs to be suspended to wait for the
   * external event.
   * <li>Step hooks up fiber with some external mechanism (like NIO channel
   * selector)
   * <li>Step returns with {@link NextAction#suspend()}.
   * <li>"External mechanism" becomes signal state and invokes
   * {@link Fiber#resume(Packet)} to wake up fiber
  ...
  public interface CompletionCallback {
     ...
     * Since the processing flow runs asynchronously, this method maybe invoked by a
     * different thread than any of the threads that started it or run a part of
     * stepline.
     * 
     * @param packet
     *          The packet
  ...
  public interface ExitCallback {
    /**
     * Indicates that a thread has finished processing the fiber, for now.  If the fiber is 
     * done or cancelled then no thread will enter the fiber again.
     */
    void onExit();
  }
   ...
   *
   * @param stepline
   *          The first step of the stepline that will act on the packet.
   * @param packet
   *          The packet to be passed to {@code Step#apply(Packet)}.
   ...
   * If a fiber was suspended without specifying the next {@link Step}, then the
   * execution will be resumed, by calling the {@link Step#apply(Packet)} method
   * on the next/first {@link Step} in the {@link Fiber}'s processing stack with
   * the specified resume packet as the parameter.
   * If a fiber was suspended with specifying the next {@link Step}, then the
   * execution will be resumed, by calling the next step's
   * {@link Step#apply(Packet)} method with the specified resume packet as the
   * parameter.
   * This method is implemented in a race-free way. Another thread can invoke this
   * method even before this fiber goes into the suspension mode. So the caller
   * need not worry about synchronizing {@link NextAction#suspend(Consumer)} and this
   * method.
   *
   * @param resumePacket
  ...
  /**
   * Similar to resume(Packet) but allowing the Fiber to be resumed and at the
   * same time atomically assign a new CompletionCallback to it.
   * 
   * @param resumePacket
   *          packet used in the resumed processing
   * @param callback
   *          Replacement completion callback
   */
  ...
  /**
   * Marks this Fiber as cancelled. A cancelled Fiber will never invoke its
   * completion callback
   * 
   * @param mayInterrupt
   *          if cancel should use {@link Thread#interrupt()}
  ...
  /**
   * Cancels the current thread and accepts a callback for when the current thread, if any,
   * exits processing this fiber.  Since the fiber will now be cancelled or done, no thread
   * will re-enter this fiber.  If the return value is true, then there is a current thread
   * processing in this fiber and the caller can expect a callback; however, if the return 
   * value is false, then there is no current thread and there will be no callback.
   * @param mayInterrupt
   *          if cancel should use {@link Thread#interrupt()}
   * @param exitCallback Callback for when current thread, if any, finally exits the fiber
   * @return true, if there is a current thread executing in the fiber and that callback will eventually occur
   */
   ...
   * This works like {@link Thread#currentThread()}. This method only works when
   * invoked from {@link Step}.
