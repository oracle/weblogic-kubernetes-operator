...
public abstract class Step {
   ...
   * 
   * @param next
   *          The next step, use null to indicate a terminal step
   */
  ...
  /**
   * Create {@link NextAction} that indicates that the next step be invoked with
   * the given {@link Packet}
   * 
   * @param packet
   *          Packet to provide when invoking the next step
  ...
  /**
   * Create {@link NextAction} that indicates that the indicated step be invoked with
   * the given {@link Packet}
   * 
   * @param step The step
   * @param packet
  ...
  /**
   * Create {@link NextAction} that indicates the the current step be retried
   * after a delay
   * 
   * @param packet
   *          Packet to provide when retrying this step
   * @param delay
   *          Delay time
   * @param unit
   *          Delay time unit
  ...
  /**
   * Create {@link NextAction} that indicates the the current fiber resume with
   * the next step after a delay.
   * 
   * @param packet
   *          Packet to provide when retrying this step
   * @param delay
  ...
  /**
   * Create {@link NextAction} that indicates the the current fiber resume with
   * the indicated step after a delay.
   * 
  ...
  /**
   * Create {@link NextAction} that suspends the current {@link Fiber}. While
   * suspended the Fiber does not consume a thread. Resume the Fiber using {@link Fiber#resume(Packet)}
   * 
  ...
  /**
   * Create {@link NextAction} that suspends the current {@link Fiber}. While
   * suspended the Fiber does not consume a thread. When the Fiber resumes it will
   * start with the indicated step. Resume the Fiber using
   * {@link Fiber#resume(Packet)}
   * 
  ...
  /**
   * Create a {@link NextAction} that suspends the current {@link Fiber} and that
   * starts child fibers for each step and packet pair.  When all of the created
   * child fibers complete, then this fiber is resumed with the indicated step
   * and packet.
   ...
   * @return Next action
   */
  protected NextAction doForkJoin(Step step, Packet packet, Collection<StepAndPacket> startDetails) {
    return doSuspend(step, (fiber) -> {
  ...
  /**
   * Simplifies creation of stepline.  Steps will be connected following the list ordering
   * of their classes
