...
public class InMemoryDatabaseTest {
  ...
  private InMemoryDatabase<V1beta1Ingress,V1beta1IngressList> database = new InMemoryDatabase<V1beta1Ingress, V1beta1IngressList>() {
    @Override
  ...
  public void whenItemPresent_createThrowsException() throws Exception {
    ...
    try {
      database.create(new V1beta1Ingress().metadata(new V1ObjectMeta().namespace(NS1).name(NAME1)), keys().namespace(NS1).map());
      fail("Should have thrown an InMemoryDatabaseException");
    } catch (InMemoryDatabaseException e) {
  ...
  private V1beta1Ingress createItem(String name, String namespace) {
    V1beta1Ingress item = new V1beta1Ingress().metadata(new V1ObjectMeta().namespace(namespace).name(name));
  ...
  public void whenItemAbsent_replaceThrowsException() throws Exception {
    try {
      database.replace(new V1beta1Ingress().metadata(new V1ObjectMeta().namespace(NS1).name(NAME1)), keys().namespace(NS1).map());
      fail("Should have thrown an InMemoryDatabaseException");
    } catch (InMemoryDatabaseException e) {
      assertThat(e.getCode(), equalTo(HttpURLConnection.HTTP_NOT_FOUND));
    }
  ...
  public void afterReplaceItem_canRetrieveNewItem() throws Exception {
    ...
    V1beta1Ingress replacement = new V1beta1Ingress().metadata(new V1ObjectMeta().namespace(NS1).name(NAME1)).kind("new item");
  ...
  public void afterItemsCreated_canListMatches() throws Exception {
    ...
    assertThat(database.list(keys().namespace(NS1).map()).getItems(), containsInAnyOrder(item1, item2));
    assertThat(database.list(keys().name(NAME1).map()).getItems(), containsInAnyOrder(item1, item3));
  }
